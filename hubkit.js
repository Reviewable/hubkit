            ...
            if (res.status === 304) {
              ...
              if (detectApi(path) === 'graph') {
                ...
                if (paginated) {
                  ...
                  if (result) {
                    if (root.nodes) resultRoot.nodes = resultRoot.nodes.concat(root.nodes);
                    if (root.edges) resultRoot.edges = resultRoot.edges.concat(root.edges);
                    for (const key in root) {
                      if (!Object.hasOwnProperty.call(root, key) ||
                          key === 'nodes' || key === 'edges' || key === 'pageInfo') {
                        continue;
                      }
