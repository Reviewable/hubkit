...
(function(init) {
  ...
  Directive.prototype.parseArgs = function(minArgs, maxArgs) {
    var args = this.arg.split(',').map(function(s) {return s.trim();});
    if (args.length >= minArgs && (!maxArgs || args.length <= maxArgs)) return args;
    this.error(
      'expected ' + (maxArgs ? minArgs + '-' + maxArgs : 'at least ' + minArgs) + ' arguments');
  };
  ...
  FieldDirective.prototype.render = function() {
    var args = this.parseArgs(2);
    var self = this;
    return reflectGraphQLType(args[0], this.hubkit).then(function(fields) {
      if (!fields) self.error('unknown type ' + args[0]);
      for (var i = 1; i < args.length; i++) {
        if (fields.include(args[i])) return args[i];
      }
      return '';
    });
  };
  ...
  function replaceAsync(str, regex, replacerFunction) {
    var promises = [];
    str.replace(regex, function(match) {
      promises.push(replacerFunction.apply(null, arguments));
      return match;
    });
