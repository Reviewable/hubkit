...
* `#field(type, ...fields)`: Gets substituted with the first given field that exists in the schema for the given type.
...
Schema information for the `#exists` and `#field` directives are queried from the server and are cached indefinitely in memory (regardless of any cache related options). 
